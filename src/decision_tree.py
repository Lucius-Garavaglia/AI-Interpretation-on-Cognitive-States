...
def build_decision_tree(max_depth=None, random_state=42):
    class_weight = 'balanced'
    return DecisionTreeClassifier(max_depth=max_depth, random_state=random_state, class_weight=class_weight)


def run_decision_tree_experiment(test_size=0.2, random_state=42, max_depth=None):
    X, y = load_processed_data()
    X_train, X_test, y_train, y_test = get_train_test_split(X, y, test_size=test_size, random_state=random_state)

    clf = build_decision_tree(max_depth=max_depth, random_state=random_state)
    clf.fit(X_train, y_train)
    y_pred = clf.predict(X_test)

    print('Decision Tree trained with max_depth=', max_depth)
